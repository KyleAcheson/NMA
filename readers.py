...
##########################################################################################################
# Author: Kyle Acheson
# A module to read trajectories from xyz files outputted by common quantum dynamics codes,               #
# predifined trajectories from .npy or .mat files                                                        #
# trajectories are always formatted to have matrix elements that are np.nan where the trajectories crash #
##########################################################################################################
...
def read_trajs_from_xyz(fpaths: str, code_type: str, natom: int) -> tuple[npt.NDArray, npt.NDArray, int]:
    # main function for reading trajectories specified in every subdir listed in the parent dir fpaths
...
def load_trajs(traj_file: str, natom:int, nts: int) -> tuple[npt.NDArray, int]:
    # main fumction for loading trajectories from an existing .mat or .npy files - not xyz
...
def read_mat(filename: str, var_names: list):
    ...
    # if .mat file - ensure only one variable is contained within it
...
###############################################
# Readers for time intervals for nma analysis #
###############################################

def get_time_intervals(time_intervals: Union[str, list], ntraj: int) -> tuple[npt.NDArray, int]:
    # main function for retrieving time intervals in suitable format
...
def format_traj_run_times(run_times: list) -> npt.NDArray:
    ...
    # into a time interval array - used when you want to
    # perform oneshot analysis over the whole trajectory run time
    ntraj = len(run_times)
